...
export function ListingWizard() {
  ...
  if (!localBrokers.length) return <main className={styles.main}><section className={styles.accessCard}><BadgeCheck size={38} /><span className={styles.eyebrow}>{verifiedOnChainBalance ? "Balance verified" : "Broker required"}</span><h1>{verifiedOnChainBalance ? "Broker metadata integration pending" : "Broker access is not available yet"}</h1><p>{verifiedOnChainBalance ? `This wallet has a verified collection balance of ${ownership.balance?.toString()}, but token IDs and metadata tiers are not available from the current contract reader. Listing creation stays locked rather than assigning an unverified allowance.` : "The Isekai Brokers Genesis mint is TBA. Listing tools will unlock after the official contract is published and an eligible Broker can be verified."}</p><Link className="button" href="/launchpad">View Launch Mint — TBA</Link><Link href="/isekai-brokers">Review Broker utility</Link></section></main>;
