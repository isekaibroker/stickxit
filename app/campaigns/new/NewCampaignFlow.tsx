...
import { createRecordId, createShortCode, getSavedListings, onRecordsChanged, saveCampaign, savedListingToMarketplaceListing, type SavedCampaign, type StickerTransform } from "@/lib/app-storage";
...
export function NewCampaignFlow({ initialListingSlug, initialSpotId, initialTemplate }: { initialListingSlug?: string; initialSpotId?: string; initialTemplate?: string }) {
  const { address, connected, correctChain, isLocalSession, startLocalSession, switchNetwork, signMessage } = useWallet();
  ...
  const campaignListings = useMemo(() => [...localListings, ...listings], [localListings]);
  ...
  useEffect(() => {
    const load = () => setLocalListings(address ? getSavedListings(address).map(savedListingToMarketplaceListing) : []);
    load();
  ...
  async function chooseArtwork(selectedFile: File | null) {
    ...
    try {
      ...
      setActionError(error instanceof Error ? error.message : "Artwork could not be stored in this browser.");
    }
  }
  ...
  async function authorizeCampaign() {
    ...
    try {
      ...
      const message = [
        ...
        `Creative: ${mode}`,
        `Created: ${createdAt}`,
        "This signature creates a local campaign record and does not transfer funds.",
      ].join("\n");
  ...
  if (!connected) return <main className={styles.flowMain}><section className={styles.accessCard}><Wallet size={34} /><span className={styles.eyebrow}>Local campaign workspace</span><h1>Create a campaign now</h1><p>Enter the browser-only demo to authorize and save campaign records without a wallet extension or payment.</p><button className="button" onClick={() => void startLocalSession()}>Enter local demo</button><Link href="/marketplace">Return to marketplace</Link></section></main>;
  ...
  if (complete) return <main className={styles.flowMain}><section className={styles.success}><CheckCircle2 size={52} /><span className={styles.eyebrow}>Local authorization saved</span><h1>Campaign ready for review</h1><p>Campaign <b>{complete.id}</b> is linked to {complete.owner.slice(0, 6)}…{complete.owner.slice(-4)}. Its dynamic link is <Link href={`/r/${complete.shortCode}`}><b>/r/{complete.shortCode}</b></Link>. No funds were transferred.</p><div className={styles.successActions}><Link href="/campaigns" className="button">Open campaign workspace</Link><Link href="/marketplace" className={styles.secondary}>Book another spot</Link></div></section></main>;
  ...
  return <main className={styles.flowMain}>
    ...
    <div className={styles.flowHead}><div><span className={styles.eyebrow}>{isLocalSession ? "Browser-only simulation" : "Wallet-linked campaign"}</span><h1>Create a campaign</h1><p>Choose a placement, build the creative, and save an authorization {isLocalSession ? "locally in this browser" : "with your connected wallet"}.</p></div><ol className={styles.steps}>{labels.map((label, i) => <li key={label} className={step >= i + 1 ? styles.stepActive : ""}><span>{i + 1}</span>{label}</li>)}</ol></div>
    <div className={styles.builder}>
      <section className={styles.panel}>
        {step === 1 && <><h2>Choose placement</h2><p className={styles.lead}>Choose a local listing or an example surface and its host-approved sticker area.</p><div className={styles.formPair}><label>Item<select value={listingSlug} onChange={(event) => { const nextListing = campaignListings.find((item) => item.slug === event.target.value) ?? campaignListings[0]; setListingSlug(nextListing.slug); setSpotId(nextListing.spots[0]?.id ?? ""); setStickerTransform(initialStickerTransform); }}>{campaignListings.map((item) => <option key={item.slug} value={item.slug}>{item.title} · {item.city}{item.slug.startsWith("local-") ? " · Local" : ""}</option>)}</select></label><label>Spot<select value={spot?.id ?? ""} onChange={(event) => { setSpotId(event.target.value); setStickerTransform(initialStickerTransform); }}>{availableSpots.map((item) => <option key={item.id} value={item.id}>Spot {item.id} · {item.name} · {item.physicalSize}</option>)}</select></label></div><div className={styles.placementChoice}><div><span className={styles.status} data-status="active">{listing.slug.startsWith("local-") ? "Local listing" : "Example"}</span><h3>{listing.title}</h3><p>{listing.city}, {listing.country}</p><b>Spot {spot?.id} · {spot?.name}</b><span>{spot?.physicalSize} host-approved print area</span></div></div></>}
        {step === 2 && <><h2>Build the creative</h2><label>Campaign name<input value={campaignName} maxLength={64} onChange={(event) => setCampaignName(event.target.value)} /></label><fieldset><legend>Creative mode</legend><div className={styles.choiceGrid}>{modes.map(value => <label key={value} className={mode === value ? styles.choiceActive : styles.choice}><input type="radio" name="mode" checked={mode === value} onChange={() => setMode(value)} />{value}</label>)}</div></fieldset>
          {mode !== "QR only" && <label className={styles.upload}><Upload /><span><b>{file || "Choose artwork"}</b><small>PNG or JPG · stored in this browser</small></span><input type="file" accept="image/png,image/jpeg" onChange={event => void chooseArtwork(event.target.files?.[0] ?? null)} /></label>}
          {mode !== "Art only" && <><label>Destination URL<input type="url" required value={url} onChange={event => setUrl(event.target.value)} placeholder="https://example.com" /></label><fieldset><legend>QR corner</legend><div className={styles.choiceGrid}>{corners.map(value => <label key={value} className={corner === value ? styles.choiceActive : styles.choice}><input type="radio" name="corner" checked={corner === value} onChange={() => setCorner(value)} />{value}</label>)}</div></fieldset><p className={styles.shortNote}>The short link is created after local authorization.</p></>}
        ...
        {step === 4 && <><h2>Authorize with your wallet</h2><p className={styles.lead}>Your signature links this local campaign record to {address?.slice(0, 6)}…{address?.slice(-4)}. It is not a blockchain transaction and cannot move funds.</p><dl className={styles.review}><div><dt>Placement</dt><dd>{spot?.name}</dd></div><div><dt>Inventory</dt><dd>Pre-launch example</dd></div><div><dt>Payment</dt><dd>Unavailable until the marketplace launches</dd></div></dl><button className="button" disabled={authorizing || !campaignName.trim()} onClick={() => void authorizeCampaign()}>{authorizing ? <><LoaderCircle className={styles.spin} size={17} /> Waiting for wallet</> : "Sign local campaign record"}</button>{actionError && <p className={styles.actionError} role="alert">{actionError}</p>}</>}
